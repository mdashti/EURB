 ...
 */
public class InformixDialectExt extends CommonHibernateDialect implements
		HibernateDialect {

	private class InformixDialectHelper extends
	...
	/**
	 * @see net.sourceforge.squirrel_sql.fw.dialects.HibernateDialect#canPasteTo(com.sharifpro.db.meta.IDatabaseObjectInfo)
	 */
	 ...
	 * specified table composed of the given column names. CREATE UNIQUE INDEX
	 * test_index ON test_table (test_field); ALTER TABLE test_table ADD
	 * CONSTRAINT PRIMARY KEY (test_field) CONSTRAINT test_constraint; alter
	 * table table_name add constraint primary key (column_names) constraint
	 * pkName
	 * 
	 * @param pkName
	...
	public String[] getAddPrimaryKeySQL(String pkName,
			...
			DatabaseObjectQualifier qualifier, SqlGenerationPreferences prefs) {
		// TODO: should also make sure that each of the columns is made
		// "NOT NULL"
		...
		return new String[] {
				DialectUtils.getAddIndexSQL(pkName, true, columns, qualifier,
						prefs, this),
	...
	public String[] getColumnNullableAlterSQL(TableColumnInfo info,
		...
		return new String[] { DialectUtils.getColumnNullableAlterSQL(info,
				this, alterClause, true, qualifier, prefs) };
	}
	...
	public String getColumnNameAlterSQL(TableColumnInfo from,
		...
		prefs.setQuoteColumnNames(false);
		return DialectUtils
				.getColumnRenameSQL(from, to, qualifier, prefs, this);
	}
	...
	public String[] getAddAutoIncrementSQL(TableColumnInfo column,
			DatabaseObjectQualifier qualifier, SqlGenerationPreferences prefs) {
		/*
		 * This is another way to do this. Modifying the column type to serial
		 * is much easier. Thanks to Doug Lawry for setting me straight!!
		 */
		...
		// Unfortunately we cannot simply call getColumnTypeAlterSQL since
		// "serial" type is 4 (which is the same
		// as java.sql.Types.Integer, so we would get back an integer column,
		// not a serial column). So we piece
		...
		result.append(DialectUtils.shapeQualifiableIdentifier(
				column.getTableName(), qualifier, prefs, this));
		result.append(" ");
	...
	/**
	 * @see net.sourceforge.squirrel_sql.fw.dialects.HibernateDialect#getAddColumnSQL(com.sharifpro.db.meta.TableColumnInfo,
	 *      net.sourceforge.squirrel_sql.fw.dialects.DatabaseObjectQualifier,
	 *      net.sourceforge.squirrel_sql.fw.dialects.SqlGenerationPreferences)
	 */
	...
	public String[] getAddForeignKeyConstraintSQL(String localTableName,
		...
		// This method no longer creates an index for the child column. Informix
		...
		/**
		 * ALTER TABLE fkTestChildTable ADD CONSTRAINT FOREIGN KEY (fkchildid)
		 * REFERENCES fkTestParentTable (parentid) constraint fk_const_name
		 */
		StringBuilder result = new StringBuilder();
		result.append(DialectUtils.ALTER_TABLE_CLAUSE);
		result.append(" ");
		result.append(DialectUtils.shapeQualifiableIdentifier(localTableName,
				qualifier, prefs, this));
		...
		result.append(")\n REFERENCES ");
		result.append(DialectUtils.shapeQualifiableIdentifier(refTableName,
				qualifier, prefs, this));
		result.append(" (");
	...
	public String[] getAddUniqueConstraintSQL(String tableName,
			...
			DatabaseObjectQualifier qualifier, SqlGenerationPreferences prefs) {
		/*
		 * ALTER TABLE <tableName> ADD CONSTRAINT UNIQUE (<columnList>)
	...
	public String[] getAlterSequenceSQL(String sequenceName, String increment,
		...
		return new String[] { DialectUtils.getAlterSequenceSQL(sequenceName,
				increment, minimum, maximum, restart, cache, cycleClause,
				qualifier, prefs, this) };
	}
	...
	public String getCreateIndexSQL(String indexName, String tableName,
			...
			DatabaseObjectQualifier qualifier, SqlGenerationPreferences prefs) {
		String access = (accessMethod == null || accessMethod
				.equalsIgnoreCase("NORMAL")) ? null : "CLUSTER";
		...
		sql.append(" ON ").append(
				DialectUtils.shapeQualifiableIdentifier(tableName, qualifier,
						prefs, this));
	...
	public String getCreateSequenceSQL(String sequenceName, String increment,
		...
		return DialectUtils.getCreateSequenceSQL(sequenceName, increment,
				minimumClause, minimum, maximumClause, maximum, start, cache,
	...
	public String getDropSequenceSQL(String sequenceName, boolean cascade,
			DatabaseObjectQualifier qualifier, SqlGenerationPreferences prefs) {
		Boolean cascadeNotSupported = null;
		return DialectUtils.getDropSequenceSQL(sequenceName,
				cascadeNotSupported, qualifier, prefs, this);
	}
	...
	public String getRenameTableSQL(String oldTableName, String newTableName,
		...
		result.append(DialectUtils.shapeQualifiableIdentifier(oldTableName,
				qualifier, prefs, this));
		result.append(" TO ");
	...
	public String getSequenceInformationSQL(String sequenceName,
			DatabaseObjectQualifier qualifier, SqlGenerationPreferences prefs) {
		/*
		 * SELECT 'unknown' as last_value, T1.max_val AS max_value, T1.min_val
		 * AS min_value, T1.cache AS cache_size, T1.inc_val AS increment_by,
		 * case T1.cycle as is_cycled FROM informix.syssequences AS T1,
		 * informix.systables AS T2 WHERE T2.tabid = T1.tabid and T2.owner =
		 * '<schema>' and T2.tabname = '<sequenceName>'
		 */
		StringBuilder result = new StringBuilder();
	...
	public String getViewDefinitionSQL(String viewName,
		...
		/*
		 * SELECT viewtext FROM informix.systables AS T1, informix.sysviews AS
		 * T2 WHERE tabname = 'a_view' AND T2.tabid = T1.tabid
		 */
		StringBuilder result = new StringBuilder();
